"""Version 11: Add extra sounds and optimize CPU usage
Jack Andrews
25/5/23"""
import pygame
...
IMG_WIDTH = 80
IMG_HEIGHT = 135
(CRASH_SFX := mixer.Sound("./assets/sounds/crash.wav")).set_volume(0.3)
(START_SFX := mixer.Sound("./assets/sounds/start.wav")).set_volume(0.3)
...
class Button:
    ...
    def __init__(self, x, y, width, height, text, fn, txt_col="white",
                 btn_col="blue", sfx=BUTTON_SFX):
        # Added sfx property to buttons
        self.x = x
        self.y = y
        self.width = width
        self.height = height
...
class Game:
    ...
    cars = {i: pygame.transform.flip(
            pygame.transform.smoothscale(
                pygame.image.load(f"./assets/car_{i}.png").convert_alpha(),
    ...
    cars_normal = {i: pygame.transform.smoothscale(pygame.image.load(
        f"./assets/car_{i}.png").convert_alpha(), [IMG_WIDTH, IMG_HEIGHT])
        for i in range(1, 7)}
    cars_stripew = {i: pygame.transform.smoothscale(pygame.image.load(
        f"./assets/car_{i}_striped_w.png").convert_alpha(),
        [IMG_WIDTH, IMG_HEIGHT])
        for i in range(1, 7)}
    cars_stripeb = {i: pygame.transform.smoothscale(pygame.image.load(
        f"./assets/car_{i}_striped_b.png").convert_alpha(),
        [IMG_WIDTH, IMG_HEIGHT])
        for i in range(1, 7)}
    # Loading all the images needed for the player
    # Added convert_alpha to reduce cpu usage
    ...
    def save_high_score(fpath, score, stop=False):
        ...
        if stop:
            LEAVE_SFX.play()
            sleep(1)
            # Apparently playing sound doesn't prevent further code from
            # executing
            pygame.quit()
            exit()
    ...
    green_btn = Button(30, HEIGHT // 2 + 100, 50, 50, '',
                       ...
                       btn_col="green", sfx=None)
    purple_btn = Button(100, HEIGHT // 2 + 100, 50, 50, '',
                        lambda: Game.car.change_colour("purple"),
                        btn_col="purple", sfx=None)
    copper_btn = Button(30, HEIGHT // 2 + 170, 50, 50, '',
                        lambda: Game.car.change_colour("copper"),
                        btn_col="copper", sfx=None)
    cyan_btn = Button(100, HEIGHT // 2 + 170, 50, 50, '',
                      lambda: Game.car.change_colour("cyan"),
                      btn_col="cyan", sfx=None)
    # These buttons correspond to the colour of the car ingame
    white_stripe_btn =\
        Button(WIDTH - 170, HEIGHT // 2 + 30, 150, 50,
               "White", lambda: setattr(Game.car, "stripe", 'w')
               or Game.car.change_img(), "black", "white", None)
    black_stripe_btn =\
        Button(WIDTH - 170, HEIGHT // 2 + 100, 150, 50,
               "Black", lambda: setattr(Game.car, "stripe", 'b')
               or Game.car.change_img(), "black", "white", None)
    no_stripe_btn =\
        Button(WIDTH - 170, HEIGHT // 2 + 170, 150, 50, "None",
