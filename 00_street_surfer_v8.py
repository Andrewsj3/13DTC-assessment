"""Version 8: Change car movement to be smooth instead of instantaneous
and make the car actually turn
Also improved car collision by using masks
Jack Andrews
13/5/23"""
import pygame
import random
...
class Game:
    ...
    cars = {i - 1: pygame.transform.flip(
            pygame.transform.smoothscale(
                pygame.image.load(f"./assets/car_{i}.png"),
                [IMG_WIDTH, IMG_HEIGHT]), False, True) for i in range(2, 7)}

    car_img = pygame.image.load("./assets/car_1.png")
    car_img = pygame.transform.smoothscale(car_img, [IMG_WIDTH, IMG_HEIGHT])
    car = Car(car_img, START_X)
    car_queue = Queue([], 4)
